// Pure math helpers — byte-identical to zombie_roguelike.html src 1598–1610
// and the lerpAngle helper at src 3547–3552.
...
export function lighten(hex: string, pct: number): string {
  ...
  const m = (v: number): number =>
    clamp(Math.floor(v + (255 - v) * pct / 100), 0, 255);
  return (
    '#' +
    [m(r), m(g), m(b)]
