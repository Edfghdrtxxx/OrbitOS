...
---
# UTokyo CNS / RIKEN Professor Outreach
...
- Target route: **Primary:** Direct PhD D1 via Route B (Oct 2027); **Fallback:** Kenkyusei (Oct 2027) → PhD D1 (Oct 2028). UTokyo CNS with RIKEN JRA funding. *(Updated 2026-03-09: Route B chosen, Route A archived; Sub-track RESOLVED: Special Selection (confirmed 2026-04-13 Imai meeting))* [^utokyo-pathway]

> [!success] Sub-track RESOLVED (2026-04-13)
> Special Selection confirmed. GSGC ineligible for master's holders. See [[DG_GSGC_vs_Special_Selection]] (RESOLVED).
...
- Personal profile: Master's student in Particle and Nuclear Physics at IMP, CAS (degree via UCAS), expected graduation June 2027; ML focus ResNet/ViT; physics focus TPC, direct reactions, PID. [^claude-profile]

## Research Fit Snapshot (from AFTPC/MATE work)
- AFTPC (Automated Framework for Time Projection Chamber) is a deep learning framework for automated classification of TPC charged-particle tracks, using 2D projections and CNN/ViT models. [^gemini-overview]
- Project context explicitly references MATE at IMP and uses physics-informed deep learning for explainability. [^gemini-mate]
- Core models include ResNet-18 and ViT (via `timm`). [^gemini-models]
- Codebase path (relative to vault root): `../../Something/AFTPC_V3_MultiAgentVersion`. [^aftpc-path]

## Candidate Professors (Ranked by Priority - Preliminary)
**Ranking criteria:** proximity to your target alignment (TPC + direct reactions + PID + ML) based on each lab's publicly stated research focus. This is an inference layer and should be validated with lab publications and direct correspondence. [^utokyo-alignment] [^yako-lab] [^imai-lab] [^yamaguchi-lab] [^sakemi-lab] [^gunji-lab]

| Priority | Name                | Title                | Lab / Research Area (from CNS)                                                                                                                  | Evidence                       | Email (as listed)                      | Outreach Status | Notes                                                                                                                                                                          |           |
| -------- | ------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ | -------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- |
| 1        | Nori Aoi            | Professor            | Exotic nuclear reactions; Nuclear spectroscopy; RI beam experiments (RIBF/OEDO/SHARAQ); development of Active Target TPCs (CAT-M). [^aoi-lab]   | CNS member list. [^cns-member] | aoi (email username). [^cns-member]    | Accepted (2026-03-09) — user committed to Imai's group (2026-04-13); Aoi accessible via same-campus collaboration   | **Strongest overall fit.** Direct alignment with TPC development (CAT-M) and exotic nuclei structure. Moved to CNS March 2025. Not primary supervisor — collaboration partner via CNS. [[Prof_Nori_Aoi]] |           |
...
| 3        | Nobuaki Imai        | Assoc. Professor     | Quantum many-body nuclear phenomena; detector development (gamma-ray, recoiled particle, ultra-thin diamond); RIB decelerator OEDO. [^imai-lab] | CNS member list. [^cns-member] | n.imai (email username). [^cns-member] | **Committed (2026-04-13)** — primary supervisor, DONUTS group. Interview only (no written exam). GRE Physics min 900. Materials before Dec 2026.   | **Primary supervisor.** Instrumentation alignment (TPC-adjacent). [[Prof_Nobuaki_Imai]] |           |
...
[^gemini-mate]: `D:\Something\AFTPC_V3_MultiAgentVersion\GEMINI.md:6`; `D:\Something\AFTPC_V3_MultiAgentVersion\GEMINI.md:7`
[^gemini-models]: `D:\Something\AFTPC_V3_MultiAgentVersion\GEMINI.md:12`
[^aftpc-path]: `20_Project/Automated Framework for MATE(TPC).md:5`; `20_Project/Automated Framework for MATE(TPC).md:7`
[^cns-member]: https://www.cns.s.u-tokyo.ac.jp/en/about/member/
