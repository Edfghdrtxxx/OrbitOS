---
title: Speaking Collocation Review — Guide & Notes
type: resource
status: active
...
---
# Speaking Collocation Review — Guide & Notes

> [!info] Scheduling is owned by the plugin
> The **Obsidian Spaced Repetition** plugin (st3v3nmw) schedules every card in
> [[Speaking_Collocation_Bank]] (SM-2 or FSRS) and writes review data as an
> `<!--SR:...-->` comment next to the card. Do not edit those comments by hand.
>
> Writing / Academic Discussion phrases: [[Writing_Phrase_Bank]] (separate note;
> not required for the speaking deck).
...
4. Open [[Speaking_Collocation_Bank]] once so the vault cache sees the tags.
5. Command palette → **Spaced Repetition: Review flashcards** → open deck **`flashcards` → `toefl` → `speaking`**.

No need to add a custom `#toefl-speaking` tag.

## Daily review (~5/day)

1. **Spaced Repetition: Review flashcards** (or ribbon icon) → deck `toefl/speaking`.
2. Read the **scenario** on the front.
3. **Recall** the phrase + model aloud in a full sentence, then flip to check.
4. Rate Again / Hard / Good / Easy — the plugin schedules the next due date.
5. Each entry yields **one** card only: **scenario → phrase + model**.
