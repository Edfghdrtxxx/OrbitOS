...
source_url: "https://x.com/aiwithmayank/status/2042878582108889265"
captured: 2026-04-11
topic: "AI-assisted reading workflow"
tags: ["notebooklm", "study-method", "reading", "ai-workflow", "zettelkasten"]
related: ["[[40_Wiki]]", "[[99_System/Prompts]]", "[[Feynman-Technique]]"]
---
# Stanford NotebookLM 6-Prompt Reading Workflow

## Summary

Upload a book PDF to NotebookLM, then run these six prompts in order. The workflow is designed to extract a book's central argument, test its assumptions, filter it through personal context, challenge it adversarially, convert it into action, and produce permanent notes.

## Key Takeaways

- Treat a book as a central claim supported by sub-arguments, not as a pile of highlights.
- Audit hidden assumptions before adopting an author's worldview as fact.
- Filter ideas through current context so "interesting" does not crowd out "useful now."
- Steelman and counter-challenge the central argument before building on it.
- Convert insight into trigger → action → measurement changes, then into atomic notes for [[40_Wiki]].
...
| 3 | Personal Relevance Filter | Generic advice that does not fit you |
| 4 | Steelman & Counter-Challenger | Unchallenged ideas you would adopt by default |
| 5 | Action Extractor | Insight that never becomes behavior |
| 6 | Permanent Note Builder | Context-bound highlights that do not compound |

## Source Snapshot

Thread by **Mayank Vora** (@aiwithmayank) on X — [source thread](https://x.com/aiwithmayank/status/2042878582108889265).
...
## Prompt 1: Core Argument Extractor

**Why it matters:** Every book has one central argument everything else serves. Most readers finish the whole thing and cannot state it in two sentences — they have been *present* for the book, not *finished* it.
...
**Payoff:** Forces the book down to a claim plus load-bearing supports, graded by evidence strength.

## Prompt 2: Assumption Auditor
...
## Prompt 3: Personal Relevance Filter
...
**OrbitOS context block to paste into this prompt:**
...
## Prompt 4: Steelman & Counter-Challenger

**Why it matters:** Every book deserves two readings — one charitable, one adversarial. Most readers only do the first, which means they adopt ideas that have not been tested. Ideas that survive a steelman challenge are worth building on; ideas that collapse under it are worth discarding *before* they shape your decisions.
...
**Payoff:** Runs the adversarial challenge so you do not have to hold both sides of the argument in your head simultaneously.

## Prompt 5: Action Extractor
...
## Prompt 6: Permanent Note Builder
...
Most reading produces highlights that make sense in context and nothing outside of it. This prompt produces insights that compound across everything else you know. That is the whole difference between *consuming* books and *learning* from them.
...
**Payoff:** Outputs atomic, cross-domain-linked, evidence-anchored, question-opening notes — the format [[40_Wiki]] atomic notes are supposed to take.

## Reusable Workflow

1. Upload the book PDF to NotebookLM.
2. Run prompts **1 → 6 in order** because each one builds on the prior context.
3. Send prompt 5's "start tomorrow" action to the relevant daily note.
4. Turn prompt 6's output into atomic notes under `40_Wiki/`, then wikilink each note into its cluster.

## Related

- [[40_Wiki]] — atomic note system; prompt 6 outputs land here.
- [[99_System/Prompts]] — reusable prompts can be adapted from this workflow.
