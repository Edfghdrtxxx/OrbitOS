...
Parse the user's request. If genuinely ambiguous (unclear target files, conflicting instructions), use `AskUserQuestion` to clarify. Do **not** restate the task or wait for confirmation — move straight to dispatch.
...
- **Brief like a colleague:** state the goal, relevant file paths, what to change, and why — the sub-agent has zero prior context.
