...
description:
  Drive the user's Chrome via CDP when a real browser is needed — login sessions, page interaction, JS-rendered content, or sites that block static fetch (Xiaohongshu, Weibo, WeChat).
  Use when the user asks to operate a webpage, scrape a login-gated or anti-bot platform, or read dynamically rendered content. Skip for ordinary search or a public URL that WebSearch, WebFetch, or Defuddle can handle.
metadata:
  author: 一泽Eze
  version: "2.4.3"
---
