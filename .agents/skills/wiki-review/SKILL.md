...
If the user skips, regenerate the dashboard (step 7) and stop.
...
After every session (including skip), regenerate `99_System/Review_Dashboard.md`.

Use the Write tool to overwrite the file with this structure:
...
- [[Note1]] (last reviewed: YYYY-MM-DD)
...
```
