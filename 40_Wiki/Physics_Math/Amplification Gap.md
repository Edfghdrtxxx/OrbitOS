...
- **Field ratio**: The amplification field $E_\text{amp} \sim 40\;\text{kV/cm}$ is roughly $30$–$50\times$ the drift field $E_\text{drift} \sim 0.5$–$1\;\text{kV/cm}$ in the conversion gap above. This large ratio ensures that virtually all electrons crossing the mesh are funneled into the avalanche region (high mesh transparency), quantified by:
$$
...
- **Fast signals**: Because the gap is so narrow, the electron component of the induced signal is collected in $\lesssim 1\;\text{ns}$ and the slower ion component drifts back to the mesh in $\sim 100\;\text{ns}$. This gives [[Micromegas]] detectors excellent [[Time Resolution]] ($\lesssim 10\;\text{ns}$) and low sensitivity to [[Pile-up]] in high-rate environments.
...
- **Gain stability**: The gain $G$ is exponentially sensitive to gap width, gas composition, temperature, and pressure. Stable operation requires gas recirculation, temperature control, and sometimes online gain monitoring via a reference source.
...
- The [[Active-Target TPC]] at FRIB uses a Micromegas amplification stage where the gap uniformity directly determines the [[Energy Resolution]] and [[Spatial Resolution]] for reconstructing low-energy nuclear reaction tracks.
