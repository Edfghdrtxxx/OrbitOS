...
*"Einstein triangle" — the geometric form of $E^{2} = (pc)^{2} + (mc^{2})^{2}$. The hypotenuse is the total energy $E$, one leg is the momentum-energy $pc$, and the other leg is the rest energy $mc^{2}$. The arc from the hypotenuse to the rest-energy leg marks the kinetic-energy contribution $K = E - mc^{2} = (\gamma - 1)mc^{2}$. Stationary particle ($p = 0$): triangle collapses to the rest-energy leg, recovering $E = mc^{2}$. Ultra-relativistic ($pc \gg mc^{2}$): hypotenuse aligns with the $pc$ leg, recovering $E \approx pc$. (CC BY-SA 4.0, Wikimedia Commons)*
