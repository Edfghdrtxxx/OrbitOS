---
area: "[[Physics]]"
tags: [nuclear-physics, gamma-interactions, radiation]
created: 2026-03-05
last_reviewed:
next_review: 2026-03-05
review_interval: 0
---
# Compton Scattering

## Definition

Compton Scattering is the inelastic scattering of a [[Gamma Ray]] (photon) off a loosely bound or free electron, in which the photon transfers part of its energy to the electron and continues at a lower energy and longer wavelength — it is the dominant photon interaction at intermediate energies (~0.2–5 MeV) in most detector materials.

## Key Points

- **Kinematics**: The scattered photon energy depends on the scattering angle \(\theta\): \(E' = E / (1 + (E/m_e c^2)(1 - \cos\theta))\); maximum energy transfer occurs at \(\theta = 180°\) (backscatter)
- **Compton edge**: In a [[Pulse Height Spectrum]], the maximum electron recoil energy produces a sharp cutoff (Compton edge) below the photopeak; the continuum between zero and the edge is the Compton plateau
- **Cross-section**: Described by the Klein-Nishina formula; the cross-section decreases with increasing photon energy and scales with the number of electrons (∝ Z)
- **Detector impact**: Compton-scattered photons that escape the detector deposit only partial energy, degrading the peak-to-total ratio in [[Gamma Spectroscopy]] and complicating [[Energy Resolution]] analysis

## Examples

- In an [[HPGe Detector]] measuring ¹³⁷Cs (662 keV), the Compton edge appears at ~478 keV and the backscatter peak at ~184 keV in the [[Pulse Height Spectrum]]
- A [[Scintillation Detector]] with small volume has a large Compton-to-photopeak ratio because many scattered photons escape before full absorption, motivating Compton-suppression shields

## Related Concepts

- [[Gamma Ray]]
- [[Photoelectric Absorption]]
- [[Pair Production]]
- [[Gamma Spectroscopy]]
- [[Pulse Height Spectrum]]
- [[Energy Resolution]]
- [[Compton Edge]]

## References

- Knoll, G. F. (2010). *Radiation Detection and Measurement*, 4th ed. Wiley — Ch. 2
- Krane, K. S. (1988). *Introductory Nuclear Physics*. Wiley — Ch. 7
