---
name: llm-council
description: EXPLICIT INVOKE ONLY — `/llm-council` or the user names this skill. NEVER auto-activate from "opinion", "discuss", "stress-test", multi-model names, high-stakes topic, or adversarial tone. Cross-LLM council (host + Grok + Gemini via omp agents): Stage 1 parallel opinions → Stage 2 peer review → Stage 3 Chairman synthesis. Inspired by karpathy/llm-council.
---
# Activation gate

- **Fire only** on `/llm-council` or an explicit ask to run this skill by name.
- **NEVER** infer this skill from: "what's your opinion", "discuss with X/Y/Z models", "stress-test", "council-like" multi-agent requests, research/thesis stakes, or naming `swe-2` / `grok-4_6` / `gemini-3_8` without `/llm-council`.
- Named-agent discussion without this skill → plain `task` dispatches (or host-only answer). Do not load this flow.
...
- **Explicit invoke only** — see Activation gate. Never self-start this skill.
