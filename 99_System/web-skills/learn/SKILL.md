...
name: learn
description: |
  Use this skill when the user wants intellectual understanding — learning how or why something works, not getting a task done or soliciting an evaluative verdict.
...
Apply: same-family → same note (Physics prereq forks still split); no hints/answer keys in notes unless asked; mixed-judgment drills (not labeled sections); no LaTeX in GFM pipe tables; Physics mastery drills → Physics GRE level (Physics only); diagnose prerequisites before the target; mid-session foundation gaps get a new note, parent↔child linked.
...
## Surface (detect once, silently)

This skill runs on grok.com. There is no OrbitOS vault unless a project, computer, or connector actually exposes those files.

**File-backed** — you can read/write project or connector files, and `60_Learning_Progress/` or `Progress-context.md` is present:
- Persist like local `/learn` (paths below).
**Chat-backed** — typical grok.com chat, no learner files:
- Chat is the teaching surface. Still persist a session document (see Record). Do not fail a turn because a vault path is missing. Do not mention vault paths to the learner.
- Ask questions in the conversation. Do not call `AskUserQuestion`, `ask_user_question`, `show_widget`, or `read_me` — those tools are not on this host.

## Before anything else (mandatory)

**File-backed:**
1. **Read the learning index first:** `60_Learning_Progress/Progress-context.md` (or any `Progress-context.md` in the workspace) — what the learner already knows, active threads, session-note pointers. Do not re-teach solid items; do not invent prior knowledge not listed there.
2. Open the matching session note (see Record below) when continuing a thread.

**Chat-backed:**
1. Do not invent prior knowledge. If a Learning index document or project file exists, read it. Otherwise one calibrating question, then teach.
2. If a session document for this topic already exists in the conversation or project, continue it.
...
**File-backed:** The **progress note is the primary surface**. Chat may mirror it; chat-only teaching is a failure. Every turn, **before** (or as) you reply in chat, append the full exchange to the session note.
...
**Only if missing:** create `60_Learning_Progress/<topic>/<YYYY-MM-DD-HHmm> <subtopic>.md` when that tree exists; otherwise the same filename at the workspace root (or in `Learning_Progress/` if you create that folder).

**Chat-backed:** Chat is the teaching surface, but a later agent must still be able to resume. Every turn, **before** (or as) you reply:
1. Create or update a session **document** (project file if you can write one; otherwise a clearly titled markdown block you keep updating) named `YYYY-MM-DD-HHmm <subtopic>` using the turn format below. Reuse an existing document for the same topic/subtopic — never a parallel one. Sibling misses of an existing thread stay on that document; Physics prerequisite forks still split.
2. After a turn that **advances durable understanding**, update a terse Learning index document (`Progress-context`) — see Learning index below.
...
**Immediately** after each turn that **advances durable understanding** (learner states/uses it correctly — not tutor exposition alone), **update** the learning index (file-backed: `60_Learning_Progress/Progress-context.md` or workspace `Progress-context.md`; chat-backed: the `Progress-context` document):
- Keep entries **terse** — one line per concept/skill when possible
- Sections: **Known solid** | **In progress** | **Session notes** (wikilink / path / document title) | optional **Gaps / next**
- Record *what they can do or state correctly*, not full tutor prose
- Subsequent `/learn` agents **must** read this index first (see above)
...
**Persist the frame.** On the first reconstruct turn, add this block near the top of the session note (file-backed) or session document (chat-backed), above the turns. Fill a field only when the learner has established it — do not pre-write the answers.
...
**grok.com default:** render the visual in chat — a markdown table, an ASCII sketch, or a code block that draws the figure. The visual carries the structure; your prose carries the teaching. Do not call `show_widget` or `read_me`. If the host can generate an image and a picture would carry the structure better than text, you may generate one — still one relationship, one step, one comparison, paired with a question in your own words.
